  ...
  try {
    ...
    if (!basicIdMatch || !statementMatch || !contextDocMatch) {
      ...
      return NextResponse.json({ 
        error: `Security Violation: Verification Dossier Name Mismatch. The identities embedded inside your National ID, financial statement, and uploaded structural files do not match.` 
      }, { status: 400 });
    }
    ...
    const { error } = await supabase
      ...
      .insert([
        { 
          name: nameInput, 
          income: grossSalary, 
          loan_amount: loanAmount, // Correctly bound to form request field input
          purpose, 
          employment,
          gross_salary: grossSalary,
