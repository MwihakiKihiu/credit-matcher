          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="p-4 font-semibold text-slate-600">Calculated Debt Capacity</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {deals.map((deal) => (
                <tr key={deal.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-4 font-medium text-slate-800">
                    KSh {Number(deal.calculated_max_emi || 0).toLocaleString()}/mo
                  </td>
                  <td className="p-4">
                    <span className={`px-2 py-1 rounded text-xs font-bold ${
                      ...
                      {deal.system_decision || "Pending Review"}
                    </span>
                  </td>
                  <td className="p-4">
                    <button 
              ...
              {deals.length === 0 && (
                <tr>
                  <td colSpan={6} className="text-center p-8 text-slate-400">No active deals processing.</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Statement Verification Analysis</h4>
                ...
                <div>
                  ...
                  <p className="text-slate-600 text-sm mt-1">Underwriting Class: <span className="font-semibold">{selectedDeal.system_decision}</span></p>
                </div>
              </div>
              <div className="flex space-x-4 pt-4 border-t border-slate-100">
                <button onClick={() => { alert("Deal Funded Successfully!"); setSelectedDeal(null); }} className="flex-1 bg-emerald-600 text-white py-3 rounded-xl font-bold hover:bg-emerald-700">Approve & Fund Asset</button>
